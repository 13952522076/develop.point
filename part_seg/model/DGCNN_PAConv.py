...
class PAConv(nn.Module):
    def __init__(self,  num_part):
        ...
        self.hidden = [[16], [16], [16], [16]]  # the hidden layers of ScoreNet
    ...
    def forward(self, x, norm_plt, cls_label, gt=None):
        ...
        x = x.permute(0, 3, 1, 2)  # b,2cin,n,k   [32, 6, 2048, 30]
        ...
        x2 = F.relu(self.bn2(x))  # [32, 64, 2048]
        ...
        x3 = F.relu(self.bn3(x))  # [32, 64, 2048]
        ...
        x4 = F.relu(self.bn4(x))  # [32, 64, 2048]
        ...
        x5 = F.relu(self.bn5(x))  # [32, 64, 2048]
        ###############
        xx = torch.cat((x1, x2, x3, x4, x5), dim=1)  # [32, 320, 2048]


        xc = F.relu(self.convt(xx))
        xc = F.adaptive_max_pool1d(xc, 1).view(B, -1)  # [32, 1024]

        cls_label = cls_label.view(B, 16, 1)
        cls_label = F.relu(self.convc(cls_label))  # [32, 64, 1]
        cls = torch.cat((xc.view(B, 1024, 1), cls_label), dim=1)
        cls = cls.repeat(1, 1, N)  # B,1088,N  [32, 1088, 2048]
        print(f"cls = cls.repeat(1, 1, N)  shape: {cls.shape}")

        x = torch.cat((xx, cls), dim=1)  # 1088+64*3   [32, 1408, 2048] 320+(1024+64)=1408
