...
class Model21001(nn.Module):
    def __init__(self, num_part=50, points=2048, embed_dim=128,
        ...
        for i in range(len(pre_blocks)):
            self.back_block_list.append(
                FeaturePropagation(
                    last_channel+self.channel_list[i+1], self.channel_list[i], back_blocks[i]
                )
            )
    ...
    def forward(self, x, norm_plt, cls_label, gt=None):
        ...
        out = torch.cat([fea_deeper, extra_info.repeat(1,1,self.points)], dim=1)
...
def model21001H(num_classes=40, **kwargs) -> Model21001:
    return Model21001(points=2048, class_num=num_classes, embed_dim=128,
                 pre_blocks=[4,4], pos_blocks=[4,4], k_neighbors=[32,32],back_blocks=[4,4],
                 reducers=[4,4], **kwargs)
