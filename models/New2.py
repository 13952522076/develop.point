"""
Add class token in LocalGather
Including:
...
class LocalGather(nn.Module):
    def __init__(self, in_channel, out_channel, **kwargs):
        ...
        self.token = nn.Parameter(torch.rand([1,1,1,in_channel]))

    def forward(self, x):  # x [b,p,k,3+c], p: selected points, k is nerighbor number
        xyz = x[:,:,0,:3]
        fea = x[:,:,:,3:]
        b,p,k,c = fea.size()
        token = self.token.expand([b,p,1,c])
        fea = torch.cat([fea,token],dim=2)
...
def new2A(num_classes=40, **kwargs: Any) -> New2:
    return New2(num_classes=num_classes, blocks=[2, 2, 2, 2], reducer=4, k_neighbors=[32,32,32,32], **kwargs)

def new2B(num_classes=40, **kwargs: Any) -> New2:
    return New2(num_classes=num_classes, blocks=[1, 1, 1, 1], reducer=4, k_neighbors=[32,32,32,32], **kwargs)

def new2C(num_classes=40, **kwargs: Any) -> New2:
    return New2(num_classes=num_classes, blocks=[1, 1, 1, 1], reducer=4, k_neighbors=[16,16,16,16], **kwargs)


def new2D(num_classes=40, **kwargs: Any) -> New2:
    return New2(num_classes=num_classes, blocks=[2, 3, 6, 4], reducer=4, k_neighbors=[32,32,32,32], **kwargs)

def new2E(num_classes=40, **kwargs: Any) -> New2:
    return New2(num_classes=num_classes, embed_channel=64, blocks=[1,1,1,1], reducer=4, k_neighbors=[32,32,32,32], **kwargs)
