"""
Based on Model8, concatenate all stage output, and using same channel
...
class LocalGrouper(nn.Module):
    ...
    def forward(self, xyz, points):
        ...
        #                        , dim=-1)
        augmented_grouped_points = grouped_points+new_points.view(B, S, 1, -1)
        return new_xyz, augmented_grouped_points
...
class PreExtraction(nn.Module):
    def __init__(self, channels, blocks=1, heads=4, dim_head=32):
        """
...
class Model12(nn.Module):
    def __init__(self, points=1024, class_num=40, embed_dim=64,
                 ...
                 heads=4, dim_head=32, reducers=[2,2,2,2], **kwargs):
        ...
        for i in range(len(pre_blocks)):
            ...
            pre_block_module = PreExtraction(embed_dim, pre_block_num, heads=heads, dim_head=dim_head)
            self.pre_blocks_list.append(pre_block_module)
            # append pos_block_list
            pos_block_module = PosExtraction(embed_dim, pos_block_num,  heads=heads, dim_head=dim_head)
...
if __name__ == '__main__':
    ...
    grouper = LocalGrouper(4,3)
    fea = torch.rand(2,10,2)
    xyz = torch.rand(2,10,3)
    new_xyz, new_fea = grouper(xyz, fea)
