"""
Based on model3, change the group operation to purely based on point feature
...
class PreExtraction(nn.Module):
    def __init__(self, channels, blocks=1, heads=8, dim_head=64):
        ...
        self.up_channel = FCBNReLU1D(channels, channels*2)
    def forward(self, x):
...
class Model4(nn.Module):
    def __init__(self, points=1024, class_num=40, embed_dim=64, heads=4,
        ...
        for i in range(len(pre_blocks)):
            ...
            pre_block_module = PreExtraction(last_channel, pre_block_num, heads=self.heads, dim_head=dim_head)
...
def model4A(num_classes=40, **kwargs) -> Model4:
    return Model4(points=1024, class_num=num_classes, embed_dim=64, heads=4,
                 pre_blocks=[3,3,3], pos_blocks=[1,1,1], k_neighbors=[32,16,16],
