"""
Based on new5: fps based on both coords and encoded features
Including:
...
class TransformerBlock(nn.Module):
    def __init__(self, dim, heads=8, dim_head=64, **kwargs):
        ...
        self.position = nn.Sequential(
            nn.Conv1d(3, dim // 8,1,bias=False),
            nn.BatchNorm1d(dim // 8),
            nn.ReLU(inplace=True),
            nn.Conv1d(dim // 8, dim,1,bias=False),
            nn.BatchNorm1d(dim),
            nn.ReLU(inplace=True)
        )
    ...
    def forward(self, x):
        ...
        pos = self.position(coords.permute(0,2,1)).permute(0,2,1)  #[n,p,dim]
...
class FPSKNNGrouper(nn.Module):
    def __init__(self, points, in_channel, knn=16, **kwargs):
        """
    ...
    def forward(self, x):
        ...
        pos = x[:, :, :3]
        fea = x[:, :, 3:]
        pos = pos + self.fea_2_pos(fea.permute(0,2,1)).permute(0,2,1)
        # sampeld_points = index_points(x, farthest_point_sample(pos.contiguous(), self.points))  # [b,points, 3]
        idx = furthest_point_sample(pos.contiguous(), self.points).long()
    ...
    def __init__(self, num_classes=40, use_normals=False, points=512,
        ...
        for stage, block_num in enumerate(blocks):
            ...
            self.groupers.append(
                FPSKNNGrouper(points=points // (reducer ** stage), knn=knn, in_channel=embed_channel*factor)
            )
...
def new5A(num_classes=40, **kwargs: Any) -> New5:
    return New5(num_classes=num_classes, blocks=[2, 2, 2, 2], reducer=4, k_neighbors=[32,32,32,32], **kwargs)

def new5B(num_classes=40, **kwargs: Any) -> New5:
    return New5(num_classes=num_classes, blocks=[1, 1, 1, 1], reducer=4, k_neighbors=[32,32,32,32], **kwargs)

def new5C(num_classes=40, **kwargs: Any) -> New5:
    return New5(num_classes=num_classes, blocks=[1, 1, 1, 1], reducer=4, k_neighbors=[16,16,16,16], **kwargs)


def new5D(num_classes=40, **kwargs: Any) -> New5:
    return New5(num_classes=num_classes, blocks=[2, 3, 6, 4], reducer=4, k_neighbors=[32,32,32,32], **kwargs)

def new5E(num_classes=40, **kwargs: Any) -> New5:
    return New5(num_classes=num_classes, embed_channel=64, blocks=[1,1,1,1], reducer=4, k_neighbors=[32,32,32,32], **kwargs)
