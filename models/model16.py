...
class FCBNReLU1DResLocal(nn.Module):
    def __init__(self, channel, knn=32, kernel_size=1, bias=False):
        super(FCBNReLU1DResLocal, self).__init__()
        self.net = nn.Sequential(
            nn.Conv1d(in_channels=channel, out_channels=channel, kernel_size=kernel_size, bias=bias),
            nn.BatchNorm1d(channel),
            nn.ReLU(inplace=True),
...
class TransformerBlock(nn.Module):
    def __init__(self, dim, heads=8, dim_head=32, **kwargs):
        ...
        self.ffn = nn.Sequential(
            nn.BatchNorm1d(dim),
            nn.Conv1d(dim, dim//4, 1, bias=False),
            nn.ReLU(inplace=True),
...
class PreExtraction(nn.Module):
    def __init__(self, channels, blocks=1,heads=8, dim_head=32, knn=32):
        """
        ...
        for _ in range(blocks):
            operation.append(
                FCBNReLU1DResLocal(channels, knn=knn)
            )
        self.operation = nn.Sequential(*operation)
        self.transformer = TransformerBlock(channels, heads=heads,dim_head = dim_head)
    def forward(self, x):
...
class PosExtraction(nn.Module):
    def __init__(self, channels, blocks=1, heads=8, dim_head=32):
        """
        ...
        self.operation = nn.Sequential(*operation)
        self.transformer = TransformerBlock(channels,  heads=8, dim_head=32)
...
class Model16(nn.Module):
    def __init__(self, points=1024, class_num=40, embed_dim=64,
                 ...
                 heads=8, dim_head=16, reducers=[2,2,2,2], **kwargs):
        ...
        for i in range(len(pre_blocks)):
            ...
            pre_block_module = PreExtraction(out_channel, pre_block_num, heads=heads, dim_head=dim_head, knn=kneighbor)
            self.pre_blocks_list.append(pre_block_module)
            # append pos_block_list
            pos_block_module = PosExtraction(out_channel, pos_block_num, heads=heads, dim_head=dim_head)
...
def model16A(num_classes=40, **kwargs) -> Model16:  # 20695MiB
...
def model16B(num_classes=40, **kwargs) -> Model16:  # 10817MiB
    return Model16(points=1024, class_num=num_classes, embed_dim=64,
                  pre_blocks=[2, 2, 2], pos_blocks=[2, 2, 2], k_neighbors=[32, 32, 32],
                  reducers=[4, 2, 2], **kwargs)


def model16C(num_classes=40, **kwargs) -> Model16:  # 25261MiB
    return Model16(points=1024, class_num=num_classes, embed_dim=64,
                 pre_blocks=[4,4,4], pos_blocks=[4,4,4], k_neighbors=[32,32,32],
                 reducers=[2,2,2], **kwargs)


def model16D(num_classes=40, **kwargs) -> Model16:  # 32305MiB
    return Model16(points=1024, class_num=num_classes, embed_dim=64,
                 pre_blocks=[2,2,2], pos_blocks=[2,2,2], k_neighbors=[48,48,48],
                 reducers=[2,2,2], **kwargs)


def model16E(num_classes=40, **kwargs) -> Model16:
    return Model16(points=1024, class_num=num_classes, embed_dim=64,
...
def model16Lite_A(num_classes=40, **kwargs) -> Model16:  # 7335MiB
    return Model16(points=1024, class_num=num_classes,embed_dim=64,
                 pre_blocks=[2,2], pos_blocks=[2,2], k_neighbors=[32,32],
                 reducers=[4,4], **kwargs)

def model16Lite_B(num_classes=40, **kwargs) -> Model16:
...
def model16Lite_C(num_classes=40, **kwargs) -> Model16:  # 7875MiB
    return Model16(points=1024, class_num=num_classes,embed_dim=64,
                 pre_blocks=[3,3], pos_blocks=[3,3], k_neighbors=[32,32],
                 reducers=[4,4], **kwargs)

def model16Lite_D(num_classes=40, **kwargs) -> Model16:  # 9767MiB
    return Model16(points=1024, class_num=num_classes,embed_dim=64,
                 pre_blocks=[3,3,3], pos_blocks=[3,3,3], k_neighbors=[32,32,16],
                 reducers=[4,4,2], **kwargs)

def model16Lite_E(num_classes=40, **kwargs) -> Model16:
    return Model16(points=1024, class_num=num_classes,embed_dim=64,
                 pre_blocks=[4,4], pos_blocks=[4,4], k_neighbors=[32,32],
                 reducers=[4,4], **kwargs)

def model16Lite_F(num_classes=40, **kwargs) -> Model16:
    return Model16(points=1024, class_num=num_classes,embed_dim=64,
                 pre_blocks=[2,3,2], pos_blocks=[2,3,2], k_neighbors=[32,16,8],
                 reducers=[4,4,4], **kwargs)


def model16Lite_G(num_classes=40, **kwargs) -> Model16:  # 7831MiB
    return Model16(points=1024, class_num=num_classes,embed_dim=48,
                 pre_blocks=[2,3,2], pos_blocks=[2,3,2], k_neighbors=[32,16,8],
                 reducers=[4,4,4], **kwargs)


def model16Lite_H(num_classes=40, **kwargs) -> Model16:  # 5745MiB
    return Model16(points=1024, class_num=num_classes,embed_dim=64,
