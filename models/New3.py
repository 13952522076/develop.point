"""
In localgather, directly using the anchor as augmentation, instead of the offset
Including:
...
class LocalGather(nn.Module):
    ...
    def forward(self, x):  # x [b,p,k,3+c], p: selected points, k is nerighbor number
        xyz = x[:,:,0,:3]
        fea = x[:,:,:,3:]
        augment = (fea[:,:,0,:]).unsqueeze(dim=-2).expand_as(fea)
        fea = torch.cat([fea, augment], dim=-1)
...
def new3A(num_classes=40, **kwargs: Any) -> New3:
    return New3(num_classes=num_classes, blocks=[2, 2, 2, 2], reducer=4, k_neighbors=[32,32,32,32], **kwargs)

def new3B(num_classes=40, **kwargs: Any) -> New3:
    return New3(num_classes=num_classes, blocks=[1, 1, 1, 1], reducer=4, k_neighbors=[32,32,32,32], **kwargs)

def new3C(num_classes=40, **kwargs: Any) -> New3:
    return New3(num_classes=num_classes, blocks=[1, 1, 1, 1], reducer=4, k_neighbors=[16,16,16,16], **kwargs)


def new3D(num_classes=40, **kwargs: Any) -> New3:
    return New3(num_classes=num_classes, blocks=[2, 3, 6, 4], reducer=4, k_neighbors=[32,32,32,32], **kwargs)

def new3E(num_classes=40, **kwargs: Any) -> New3:
    return New3(num_classes=num_classes, embed_channel=64, blocks=[1,1,1,1], reducer=4, k_neighbors=[32,32,32,32], **kwargs)
