"""
Base on model7, up-channel is take from pointnet2.
...
class Attention(nn.Module):
    ...
    def forward(self, x):
        x_dim = len(x.shape)
        if x_dim==3:  # for processing both pre and pos blocks
            x = x.unsqueeze(dim=0)
        z, b, n, _, h = *x.shape, self.heads
        qkv = self.to_qkv(x).chunk(3, dim = -1)
        q, k, v = map(lambda t: rearrange(t, 'z b n (h d) -> z b h n d', h = h), qkv)

        dots = einsum('z b h i d, z b h j d -> z b h i j', q, k) * self.scale

        attn = self.attend(dots)

        out = einsum('z b h i j, z b h j d -> z b h i d', attn, v)
        out = rearrange(out, 'z b h n d -> z b n (h d)')
        if x_dim == 3:
...
class TransformerBlock(nn.Module):
    def __init__(self, dim, heads=8, dim_head=32, **kwargs):
        ...
        self.pre_att_norm = nn.LayerNorm(dim)
    ...
    def forward(self, x):
        ...
        """
        identity = x
        att = self.attention(self.pre_att_norm(x))
        att = att + identity
        out = self.ffn(self.pre_ffn_norm(att))
        out = out + att
        return out
...
class PreExtraction(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x.max(dim=2,keepdim=False)[0]  #[b, g, d]
        return x
...
class Model7(nn.Module):
    def __init__(self, points=1024, class_num=40, embed_dim=64, heads=4, dim_head=32,
        ...
        for i in range(len(pre_blocks)):
            ...
            pre_block_module = PreExtraction(out_channel, pre_block_num, heads=self.heads, dim_head=self.dim_head)
            self.pre_blocks_list.append(pre_block_module)
            local_token = nn.Parameter(torch.rand([1,1,1,out_channel]))
            self.local_token_list.append(local_token)
            # append pos_block_list
            pos_block_module = PosExtraction(out_channel, pos_block_num, heads=self.heads, dim_head=self.dim_head)
    ...
    def forward(self, x):
        ...
        for i in range(self.stages):
            ...
            global_token = (self.global_token_list[i]).expand([b,1,d])
...
def model7A(num_classes=40, **kwargs) -> Model7:
    return Model7(points=1024, class_num=num_classes, embed_dim=64, heads=4, dim_head=32,
                 pre_blocks=[2,2,2], pos_blocks=[2,2,2], k_neighbors=[32,32,32],
                 reducers=[2,2,2], **kwargs)

def model7B(num_classes=40, **kwargs) -> Model7:
    return Model7(points=1024, class_num=num_classes, embed_dim=64, heads=4, dim_head=32,
                 pre_blocks=[2,2,2], pos_blocks=[2,2,2], k_neighbors=[16,16,16],
                 reducers=[2,2,2], **kwargs)

def model7C(num_classes=40, **kwargs) -> Model7:
    return Model7(points=1024, class_num=num_classes, embed_dim=64, heads=4, dim_head=24,
                 pre_blocks=[2,2,2], pos_blocks=[2,2,2], k_neighbors=[32,32,32],
                 reducers=[2,2,2], **kwargs)

def model7D(num_classes=40, **kwargs) -> Model7:
    return Model7(points=1024, class_num=num_classes, embed_dim=64, heads=4, dim_head=32,
                 pre_blocks=[3,3,3], pos_blocks=[1,1,1], k_neighbors=[32,32,32],
                 reducers=[2,2,2], **kwargs)

def model7E(num_classes=40, **kwargs) -> Model7:
    return Model7(points=1024, class_num=num_classes, embed_dim=64, heads=4, dim_head=32,
                 pre_blocks=[2,2,2], pos_blocks=[2,2,2], k_neighbors=[16,16,16],
                 reducers=[4,2,2], **kwargs)

def model7F(num_classes=40, **kwargs) -> Model7:
    return Model7(points=1024, class_num=num_classes, embed_dim=64, heads=4, dim_head=32,
                 pre_blocks=[2,2,2], pos_blocks=[2,2,2], k_neighbors=[24,24,24],
                 reducers=[2,2,2], **kwargs)

def model7G(num_classes=40, **kwargs) -> Model7:
    return Model7(points=1024, class_num=num_classes, embed_dim=64, heads=4, dim_head=32,
                 pre_blocks=[3,3,3], pos_blocks=[1,1,1], k_neighbors=[32,16,8],
                 reducers=[2,2,2], **kwargs)

def model7H(num_classes=40, **kwargs) -> Model7:
    return Model7(points=1024, class_num=num_classes, embed_dim=64, heads=4, dim_head=32,
...
if __name__ == '__main__':
    ...
    xyz = torch.rand(1,10,3)
    grouper = LocalGrouper(3,4)
    new_xyz, new_fea = grouper(xyz, feature)
    ...
    print("===> testing model7A ...")
    model = model7A()
    out = model(data)
    print(out.shape)

    print("===> testing model7B ...")
    model = model7B()
    out = model(data)
    print(out.shape)

    print("===> testing model7C ...")
    model = model7C()
    out = model(data)
    print(out.shape)

    print("===> testing model7D ...")
    model = model7D()
    out = model(data)
    print(out.shape)

    print("===> testing model7G ...")
    model = model7G()
    out = model(data)
    print(out.shape)

    print("===> testing model7H ...")
    model = model7H()
    out = model(data)
    print(out.shape)
