"""
Based on Combine1, add more layers in classifier
Including:
...
def combine12A(num_classes=40, **kwargs: Any) -> Combine12:
    return Combine12(num_classes=num_classes, blocks=[1, 1, 1, 1], reducer=4, **kwargs)

def combine12Amax(num_classes=40, **kwargs: Any) -> Combine12:
    return Combine12(num_classes=num_classes, blocks=[1, 1, 1, 1], reducer=4, pool="max", **kwargs)

def combine12Bmax(num_classes=40, **kwargs: Any) -> Combine12:
    return Combine12(num_classes=num_classes, blocks=[1, 1, 1, 1], reducer=4, embed_channel=64, pool="max", **kwargs)

def combine12Cmax(num_classes=40, **kwargs: Any) -> Combine12:
    return Combine12(num_classes=num_classes, blocks=[1, 1, 1, 1],
...
if __name__ == '__main__':
    ...
    pointsformer = combine12A()
    data = torch.rand(2, 3, 1024)
    out = pointsformer(data)
    print(out.shape)

    print("===> testing combin1 ...")
    pointsformer = combine12Amax()
    data = torch.rand(2, 3, 1024)
    out = pointsformer(data)
    print(out.shape)

    print("===> testing combin1 ...")
    pointsformer = combine12Cmax()
    data = torch.rand(2, 3, 1024)
    out = pointsformer(data)
    print(out.shape)
