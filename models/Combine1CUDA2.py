"""
Based on Combine1CUDA, add more layers in classifier
Including:
...
class Combine1CUDA2(nn.Module):
    def __init__(self, num_classes=40, use_normals=False, points=512,
        ...
        super(Combine1CUDA2, self).__init__()
        ...
        self.classify = nn.Sequential(
            nn.Linear(embed_channel * factor * expansion, 512),
            nn.BatchNorm1d(512),
...
def combine1ACUDA2(num_classes=40, **kwargs: Any) -> Combine1CUDA2:
    return Combine1CUDA2(num_classes=num_classes, blocks=[1, 1, 1, 1], reducer=4, **kwargs)

def combine1AmaxCUDA2(num_classes=40, **kwargs: Any) -> Combine1CUDA2:
    return Combine1CUDA2(num_classes=num_classes, blocks=[1, 1, 1, 1], reducer=4, pool="max", **kwargs)

def combine1BmaxCUDA2(num_classes=40, **kwargs: Any) -> Combine1CUDA2:
    return Combine1CUDA2(num_classes=num_classes, blocks=[1, 1, 1, 1], reducer=4, embed_channel=64, pool="max", **kwargs)

def combine1CmaxCUDA2(num_classes=40, **kwargs: Any) -> Combine1CUDA2:
    return Combine1CUDA2(num_classes=num_classes, blocks=[1, 1, 1, 1],
...
if __name__ == '__main__':
    ...
    pointsformer = combine1ACUDA2()
    data = torch.rand(2, 3, 1024)
    out = pointsformer(data)
    print(out.shape)

    print("===> testing combin1 ...")
    pointsformer = combine1AmaxCUDA2()
    data = torch.rand(2, 3, 1024)
    out = pointsformer(data)
    print(out.shape)

    print("===> testing combin1 ...")
    pointsformer = combine1CmaxCUDA2()
    data = torch.rand(2, 3, 1024)
    out = pointsformer(data)
    print(out.shape)
